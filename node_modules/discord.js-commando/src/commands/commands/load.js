...
module.exports = class LoadCommandCommand extends Command {
	constructor(client) {
		super(client, {
			...
			group: 'commands',
			memberName: 'load',
			description: 'Loads a new command. (Bot owners only)',
			details: oneLine`
	...
	async run(msg, args) {
		...
		if(this.client.shard) {
			try {
				...
				await msg.channel.send(`<@` + msg.author.id + `> Loaded \`${command.name}\` command, but failed to load on other shards.`);
				return null;
			}
		}

		await msg.channel.send(`<@` + msg.author.id + `> Loaded \`${command.name}\` command${this.client.shard ? ' on all shards' : ''}.`).catch(error => console.log("Send Error - " + error));
		return null;
	}
};
