...
function grab_data(anon_id)
{
    ...
    var search_term2 = "lonely-anime-pat";
...
class PatCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "pat",
            description: "Pat yourself or another user.",
            examples: ["`!pat`", "`!pat @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            if(gifs != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***patted*** <@" + userID + ">", {
                    files: [url]
                }).catch(function (err) {
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***gave*** <@" + userID + "> ***a head pat.***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***patted themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***patted themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
