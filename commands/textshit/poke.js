...
function grab_data(anon_id)
{
    ...
    var search_term2 = "self-anime-poke";
...
class PokeCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "poke",
            description: "Poke yourself or another user.",
            examples: ["`!poke`", "`!poke @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            ...
            else
            {
                if(gifs != null)
                {
                    ...
                    message.channel.send("<@" + message.author.id + "> ***poked*** <@" + userID + ">", {
                        files: [url]
                    }).catch(function (err) {
                ...
                {
                    message.channel.send("<@" + message.author.id + "> ***gave*** <@" + userID + "> ***a poke.***").catch(function (err) {
                        message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                        console.log(err.message);
                    });
                    httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***poked themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***poked themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
