...
function grab_data(anon_id)
{
    ...
    var search_term2 = "cheek-anime-slap";
...
class SlapCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "slap",
            description: "Slap yourself or another user.",
            examples: ["`!slap`", "`!slap @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            ...
            else
            {
                if(gifs != null)
                {
                    ...
                    message.channel.send("<@" + message.author.id + "> ***slapped*** <@" + userID + ">", {
                        files: [url]
                    }).catch(function (err) {
                ...
                {
                    message.channel.send("<@" + message.author.id + "> ***gave*** <@" + userID + "> ***a slap.***").catch(function (err) {
                        message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                        console.log(err.message);
                    });
                    httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***slapped themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***slapped themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
