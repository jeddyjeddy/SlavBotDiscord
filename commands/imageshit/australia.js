...
class AustraliaCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            examples: ["`!australia`", "`!australia avatar`", "`!australia @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(args.toString().toLowerCase() != "avatar" && !otherUser)
        {
            ...
            .then(messages => {
                ...
                if(messageID == "")
                {
                    message.channel.send("<@" + message.author.id + "> No image found, use `" + commandPrefix + "help australia` for help.").catch(error => {console.log("Send Error - " + error); });
                    
                    return;
                }
                message.channel.send("***taking image***").catch(error => {console.log("Send Error - " + error); });
                Jimp.read("australia.png").then(function (australiaImage) {
                    console.log("got image");
                    Jimp.read(url).then(function (userImage) {
                        console.log("got avatar");

                        australiaImage.cover(userImage.bitmap.width, userImage.bitmap.height).opacity(0.25);
                        userImage.composite(australiaImage, 0, 0)
...
module.exports = AustraliaCommand;
