...
class FactsCommand extends command.Command
 {
    ...
    async run(message, args)
    {
        ...
        {
            if(args.length > 0)
             message.channel.send("<@" + message.author.id + "> Character limit for the text parameter is 80 characters, use `" + commandPrefix + "help facts` for help.").catch(error => {console.log("Send Error - " + error); });
            else
