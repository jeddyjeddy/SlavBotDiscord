...
function grab_data(anon_id)
{
    ...
    var search_term2 = "lonely-anime-cuddle";
...
class CuddleCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "cuddle",
            description: "Cuddle yourself or another user.",
            examples: ["`!cuddle`", "`!cuddle @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            if(gifs != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***cuddled*** <@" + userID + ">", {
                    files: [url]
                }).catch(function (err) {
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***gave*** <@" + userID + "> ***a cuddle.***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***cuddled themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***cuddled themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
