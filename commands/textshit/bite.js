...
function grab_data(anon_id)
{
    ...
    var search_term2 = "lonely-anime-bite";
...
class BiteCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "bite",
            description: "Bite yourself or another user.",
            examples: ["`!bite`", "`!bite @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            if(gifs != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***bit*** <@" + userID + ">", {
                    files: [url]
                }).catch(function (err) {
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***bit*** <@" + userID + ">").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***bit themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***bit themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
