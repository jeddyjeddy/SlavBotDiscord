...
module.exports = class PingCommand extends Command {
	...
	async run(msg, args) {
		...
		if(options == "all" && msg.author.id == msg.client.owners[0].id)
		{
			...
			msg.client.shard.fetchClientValues('shard.ready')
			.then(results => {
		...
		else if (options.startsWith("restart") && msg.author.id == msg.client.owners[0].id)
		{
			if(options == "restart all")
			{
				msg.channel.send("Restarting all shards").then(() => {
					msg.cient.shard.send(JSON.stringify({respawnAlert: "all"}))
			...
			else
			{
				...
				if(amount.length > 0)
				{
					msg.cient.shard.send(JSON.stringify({respawnAlert: "few", amount: amount}))
				}
			}
		}
		else
