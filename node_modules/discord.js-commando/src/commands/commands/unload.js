	...
	async run(msg, args) {
		...
		if(this.client.shard) {
			try {
				...
				await msg.channel.send(`<@` + msg.author.id + `> Unloaded \`${args.command.name}\` command, but failed to unload on other shards.`);
				return null;
			}
		}

		await msg.channel.send(`<@` + msg.author.id + `> Unloaded \`${args.command.name}\` command${this.client.shard ? ' on all shards' : ''}.`);
		return null;
	}
};
