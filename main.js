...
const Manager = new ShardingManager('./index.js', { token: process.env.BOT_TOKEN, totalShards: 8, respawn: true });
Manager.spawn();
    ...
    Manager.shards.forEach(shardToUse => {
        if(message.toString().indexOf("token2") > -1)
        {
            if(shardToUse.id == shard.id + 1)
            {
                ...
                shardToUse.eval(`this.fetchUser("${data["user"]}").then(user => {user.send("Thank you for voting, you have received ${data["token1"]} tokens, you will receive ${data["nextToken"]} with your next voting streak (Your voting streak is now ${data["streaks"]} out of 30, voting within the next 24 hours will increase your voting streak, each voting streak adds more tokens). You now have ${data["token2"]} tokens. You can now use the \`dailyspin\` command. Use \`help ww\`, \`help warslave\` or \`help warfare\` for more info on these tokens and \`help dailyspin\` for info on Daily Spins.\n\nYou can also purchase tokens on our website. Special weekend sales on every Friday, Saturday and Sunday.\nhttps://slavbot.com/shop").catch(error => console.log("Send Error - " + error));}, rejection => {this.shard.send(${JSON.stringify(data)});});`)
            }
        }
