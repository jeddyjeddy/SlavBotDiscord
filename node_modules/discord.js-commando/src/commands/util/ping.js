...
module.exports = class PingCommand extends Command {
	...
	async run(msg, args) {
		...
		if(options == "all" && msg.author.id == msg.client.owners[0].id)
		{
			...
			msg.client.shard.fetchClientValues('status')
			.then(results => {
				var pingAll = "***All Shards Status:***"
				for(var shard in results)
				{
					pingAll = pingAll + `\nShard ${shard} from ${totalShards}. Ready status: ${results[shard]}.`
				}
		...
		else if (options.startsWith("restart") && msg.author.id == msg.client.owners[0].id)
		{
			...
			{
				msg.channel.send("Restarting all shards").then(() => {
					msg.client.shard.send(JSON.stringify({respawnAlert: "all"}))
			...
			else
			{
				...
				if(amount.length > 0)
				{
					...
					{
						if(amount[i] < totalShards) {
						 	msg.channel.send("Restarting Shard " + amount[i])
						}
					}
					msg.client.shard.send(JSON.stringify({respawnAlert: "few", amount: amount}))
				}
			}
		}
		else
