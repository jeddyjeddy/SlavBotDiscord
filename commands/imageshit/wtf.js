...
class WTFCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
    ...
    async run(message, args)
    {
        ...
        if(args.toString().toLowerCase() != "avatar" && !otherUser)
        {
            ...
            .then(messages => {
                ...
                if(messageID == "")
                {
                    message.channel.send("<@" + message.author.id + "> No image found, use `" + commandPrefix + "help wtf` for help.").catch(error => {console.log("Send Error - " + error); });
