...
function grab_data(anon_id)
{
    ...
    var search_term2 = "self-anime-lick";
...
class LickCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "textshit",
            memberName: "lick",
            description: "Lick yourself or another user.",
            examples: ["`!lick`", "`!lick @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            ...
            else
            {
                if(gifs != null)
                {
                    ...
                    message.channel.send("<@" + message.author.id + "> ***licked*** <@" + userID + ">", {
                        files: [url]
                    }).catch(function (err) {
                ...
                {
                    message.channel.send("<@" + message.author.id + "> ***licked*** <@" + userID + ">").catch(function (err) {
                        message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                        console.log(err.message);
                    });
                    httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***licked themselves***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***licked themselves***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
