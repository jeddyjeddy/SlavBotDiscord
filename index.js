...
var DatabaseFunctions = {
    commandCounterChange: function(userID){
        if(!signedIntoFirebase || userCommandUsage === [{key: "Key", data: {uses: 0, requestsSent: 0, usesCheck: 250}}])
        {
            return;
        }
        ...
        setImmediate(() => {
            ...
            if(!isStored)
            {
                firebase.database().ref("usersettings/" + userID).once('value').then(function(snapshot) {
                    ...
                    {
                        var data = {key: userID, data: {uses: 1, requestsSent: 0, usesCheck: 250}};
    ...
    getUserCommandCounter: function(userID) {
        ...
        if(signedIntoFirebase && userCommandUsage !== [{key: "Key", data: {uses: 0, requestsSent: 0, usesCheck: 250}}])
        {
            firebase.database().ref("usersettings/" + userID).once('value').then(function(snapshot) {
                ...
                {
                    userCommandUsage.push({key: userID, data: {uses: 0, requestsSent: 0, usesCheck: 250}});
                }
              })
        }
    ...
    getLeaderboardRankings: function()
    {
        ...
        console.log("LEADERBOARD USER LENGTH - " + leaderboardRankings.length)
...
bot.on("guildMemberUpdate", (oldMemberData, newMemberData) => {
    if(newMemberData.guild.id == supportServerID)
    {
        ...
        for(var i = 0; i < oldRoles.length; i++)
        {
            ...
            if(oldRoles[i].id == shashlikSupplier)
            {
                newShashlikSupporter = false;
            }
        ...
        if(newGopnikSupporter || newSlavSupporter || newPremiumSupporter)
        {
            ...
            for(var i = 0; i < newRoles.length; i++)
            {
                if(newRoles[i].id == gopnikRole)
                {
                    ...
                    {
                        newMemberData.user.send("Thank you for upgrading! Your benefits for ***" + premiumRoleName + "*** tier have been added. You will now receive 500 Million war tokens every week.").catch(error => console.log("Send Error - " + error));
                    }
                }
                                ...
                                channel.fetchMessages().then((messages) => {
                                    ...
                                    Promise.all(promises).then(() => {
                                        ...
                                        else
                                        {
                                            ...
                                            setTimeout(() => {
                                                newMemberData.user.send("Thank you for supporting Slav Bot! You have been given the ***" + roleName + "*** role. Your name should be added on the *hall-of-slavs* channel in Slav Support. You have also been given 10 Billion War Tokens for the World War and War Slave games and will receive this every week as long as you continue to be a patron.\n\nLeaving the support server means being unable to receive these benefits, you must join back to regain them.").catch(error => console.log("Send Error - " + error));
                                            }, 500)
                                        }
        ...
        if(newBlyComradesSupporter || newBanditSupporter || newBabushkaSupporter || newSemechkiSupporter || newShashlikSupporter || newVodkaSupporter || newNitroRole)
        {
            ...
            for(var i = 0; i < newRoles.length; i++)
            {
                ...
                {
                    DatabaseFunctions.addUserTokens(newMemberData.user.id, 100000000)
                    const roleName = newRoles[i].name
                    setTimeout(() => {
                        newMemberData.user.send("Thank you for donation! You have been given the ***" + roleName + "*** role. You have also been given 100 Million War Tokens.").catch(error => console.log("Send Error - " + error));
                    }, 500)
                }
                ...
                {
                    DatabaseFunctions.addUserTokens(newMemberData.user.id, 500000000)
                    const roleName = newRoles[i].name
                    setTimeout(() => {
                        newMemberData.user.send("Thank you for donation! You have been given the ***" + roleName + "*** role. You have also been given 500 Million War Tokens.").catch(error => console.log("Send Error - " + error));
                    }, 500)
                }

                if(newSemechkiSupporter && newRoles[i].id == semechkiSquad)
                {
                    DatabaseFunctions.addUserTokens(newMemberData.user.id, 5000000000)
                    const roleName = newRoles[i].name
                    setTimeout(() => {
                        newMemberData.user.send("Thank you for donation! You have been given the ***" + roleName + "*** role. You have also been given 5 Billion War Tokens.").catch(error => console.log("Send Error - " + error));
                    }, 500)
                }

                if(newShashlikSupporter && newRoles[i].id == shashlikSupplier)
                {
                    DatabaseFunctions.addUserTokens(newMemberData.user.id, 25000000000)
                    const roleName = newRoles[i].name
                    setTimeout(() => {
                        newMemberData.user.send("Thank you for donation! You have been given the ***" + roleName + "*** role. You have also been given 25 Billion War Tokens.").catch(error => console.log("Send Error - " + error));
                    }, 500)
                }

                if(newVodkaSupporter && newRoles[i].id == vodkaBrigade)
                {
                    DatabaseFunctions.addUserTokens(newMemberData.user.id, 100000000000)
                    const roleName = newRoles[i].name
                    setTimeout(() => {
                        newMemberData.user.send("Thank you for donation! You have been given the ***" + roleName + "*** role. You have also been given 100 Billion War Tokens.").catch(error => console.log("Send Error - " + error));
                    }, 500)
                }
                
                if(newNitroRole && newRoles[i].id == nitroRole)
                {
                    const newMemberID = newMemberData.user.id;
                    const roleName = newRoles[i].name
                    firebase.database().ref("nitroboosts/" + newMemberID).once("value", (snapshot) => {
                        if(snapshot.val() == null)
                        {
                            DatabaseFunctions.addUserTokens(newMemberID, 50000000000)
                            setTimeout(() => {
                            ...
                            var timestamp = (new Date()).toJSON();
                            firebase.database().ref("nitroboosts/" + newMemberID).set(timestamp)
                        }
                        else
                        {
                            var timestamp = new Date(snapshot.val())
                            var today = new Date()
                            if(timestamp.getMonth() != today.getMonth())
                            {
                                DatabaseFunctions.addUserTokens(newMemberID, 50000000000)
                                setTimeout(() => {
                                    newMemberData.user.send("Thank you for boosting the server! You have been given the ***" + roleName + "*** role. You have also been given 50 Billion War Tokens.").catch(error => console.log("Send Error - " + error));
                            ...
                            {
                                newMemberData.user.send("Thank you for boosting the server! You had previously received tokens for boosting this month. You will get your next reward when you boost next month.").catch(error => console.log("Send Error - " + error));
                            }
                        }
                    })
                            ...
                            for(var i = 0; i < data.data.length; i++)
                            {
                                if(data.data[i].time !== null)
                                {
                                    ...
                                    if(member != undefined && botMember != undefined)
                                    {
                                        ...
                                        else if(date.getTime() - (new Date()).getTime() <= 46656000000)
                                        {
            ...
            if(userMessageCount[i].userID == user.id)
            {
                ...
                else
                {
                    ...
                    if(userMessageCount[i].messages % 1000 == 0)
                    {
                        ...
                        setTimeout(() => {
                            channel.send("<@" + user.id + "> You have sent " + numberWithCommas(messageCount) + " messages on the Social/VIP channels on the Support Server. You have been given 10k War Tokens. You will be awarded another 10k War Tokens when you reach the next 1,000 message mark and 1k tokens for every 100 messages.\n\nYou can send `@Slav Bot message counter` to check the number of messages you have sent at any time.").catch(error => console.log("Send Error - " + error));	
                        }, 500)
                    }
                    else if(userMessageCount[i].messages % 100 == 0)
                    {
                        ...
                        const messageCount = userMessageCount[i].messages
                        setTimeout(() => {
                            channel.send("<@" + user.id + "> You have sent " + numberWithCommas(messageCount) + " messages on the Social/VIP channels on the Support Server. You have been given 1k War Tokens. You will be awarded with 1k tokens for every 100 messages and 10k War Tokens when you reach the next 1,000 message mark.\n\nYou can send `@Slav Bot message counter` to check the number of messages you have sent at any time.").catch(error => console.log("Send Error - " + error));	
                        }, 500)
                    }
                }
            }
            ...
            if(reaction.emoji.name == '🛒' && reaction.message.embeds.length > 0 && reaction.message.author.id == bot.user.id && !user.bot)
            {
                ...
                if(amount.length > 0)
                {
                    ...
                    reaction.message.guild.fetchMember(user).then(function(member){
                        ...
                        if(hasRole)
                        {
                            member.send("", {embed: {title: `***${name} Already Purchased***`, description: "You have already purchased ***" + name + ".***", thumbnail: {url: bot.user.avatarURL}, color: 16711680, footer: {icon_url: bot.user.avatarURL}}}).catch(error => console.log("Send Error - " + error));
                        ...
                        else
                        {
                            if(!settingsInit)
                            {
                                member.send("", {embed: {title: `***Slav Bot Initializing***`, description: "Slav Bot has recently restarted, please try purchasing the role again after a minute.", thumbnail: {url: bot.user.avatarURL}, color: 16711680, footer: {icon_url: bot.user.avatarURL}}}).catch(error => console.log("Send Error - " + error));
                            }
                            else
                            {
                                    ...
                                    for(var channelIndex = 0; channelIndex < customCounterData[index].counters[dataIndex].channels.length; channelIndex++)
                                    {
                                        if(customCounterData[index].counters[dataIndex].channels[channelIndex].id == message.channel.id)
                                        {
                                            ...
                                            if(customCounterData[index].counters[dataIndex].channels[channelIndex].counter >= customCounterData[index].counters[dataIndex].channels[channelIndex].lastCounter + limit)
                                            {
                                                ...
                                                response = response.replace(/{USER}/g, "<@" + message.author.id + ">");
                                                message.channel.send(response).catch(error => console.log("Send Error - " + error))
                                                customCounterData[index].counters[dataIndex].channels[channelIndex].lastCounter = customCounterData[index].counters[dataIndex].channels[channelIndex].counter + limit;
                                            }
                                            else
                                            {
                                                message.channel.send(trigger + " counter: " + customCounterData[index].counters[dataIndex].channels[channelIndex].counter).catch(error => console.log("Send Error - " + error))
        ...
        if(guilds[i].id == supportServerID)
        {
            ...
            guild.fetchMembers().then(() => {
                ...
                members.forEach(member => {
                    ...
                    if(tier == 2)
                    {
                        DatabaseFunctions.addUserTokens(member.id, 50000000000)
                        setTimeout(() => {
                            member.send("You have been given your weekly payment of 50 Billion War Tokens for the " + ordinalSuffix(today.getDate()) + " of " + monthNames[today.getMonth()] + " " + today.getFullYear() + ". Your next payment will be 7 days later. Thank you for supporting Slav Bot.").catch(error => console.log("Send Error - " + error));
                        }, 500)
                    }
                    else if(tier == 1)
                    {
                        DatabaseFunctions.addUserTokens(member.id, 25000000000)
                        setTimeout(() => {
                            member.send("You have been given your weekly payment of 25 Billion War Tokens for the " + ordinalSuffix(today.getDate()) + " of " + monthNames[today.getMonth()] + " " + today.getFullYear() + ". Your next payment will be 7 days later. Thank you for supporting Slav Bot.").catch(error => console.log("Send Error - " + error));
                        }, 500)
                    }
                    else if(tier == 0)
                    {
                        DatabaseFunctions.addUserTokens(member.id, 10000000000)
                        setTimeout(() => {
                            member.send("You have been given your weekly payment of 10 Billion War Tokens for the " + ordinalSuffix(today.getDate()) + " of " + monthNames[today.getMonth()] + " " + today.getFullYear() + ". Your next payment will be 7 days later. Thank you for supporting Slav Bot.").catch(error => console.log("Send Error - " + error));
                        }, 500)
                    }
                });
                    ...
                    body.push(chunk);
                        ...
                        if(body != [] && body !== undefined && body !== null)
                        {
                            ...
                            setImmediate(() => {
                                ...
                                const amountToGive = giveawayToken
                                var resetStreaks = true, dataFound = false;
                                ...
                                Promise.all(promises).then(() => {
                                    ...
                                    var newAmountToGive = amountToGive
                                    for(var streakIndex = 0; streakIndex < streaks.length; streakIndex++)
                                    {
                                    ...
                                    DatabaseFunctions.addUserTokens(userID, newAmountToGive);
                                    firebase.database().ref("usersettings/" + userID + "/lastvote").set(JSON.stringify(timestamp.toJSON()))
                                    
                                    bot.fetchUser(userID).then(user => {
                                        user.send("Thank you for voting, you have received " + numberWithCommas(newAmountToGive) + " tokens, you will receive " + numberWithCommas(newAmountToGive + streakToken) + " with your next voting streak (Your voting streak is now " + currentStreaks + " out of 30, voting within the next 24 hours will increase your voting streak, each voting streak adds more tokens). You now have " + numberWithCommas(DatabaseFunctions.getUserTokens(user.id)) + " tokens. You can now use the `dailyspin` command. Use \`help tokens\`, \`help ww\`, \`help warslave\`, \`help warfare\` or \`help cc\` for more info on these tokens and `help dailyspin` for info on Daily Spins.\n\nYou can also purchase tokens on our website. Special weekend sales on every Friday, Saturday and Sunday.\nhttps://slavbot.com/shop").catch(error => console.log("Send Error - " + error));
                                    }, rejection => {
                                        var messageData = JSON.stringify({"user": user.id, "streaks": currentStreaks, "nextToken" : numberWithCommas(newAmountToGive + streakToken), "token1": numberWithCommas(newAmountToGive), "token2" : numberWithCommas(DatabaseFunctions.getUserTokens(user.id))})
                                        bot.shard.send(messageData)
                                    });
                                })
                            })
