		...
		if(args.command && !showAll) {
			if(commands.length === 1) {
				...
				try {
					...
					if(msg.channel.type !== 'dm') messages.push(await msg.channel.send('<@' + msg.author.id + '> Sent you a DM with information.'));
				} catch(err) {
					messages.push(await msg.channel.send('<@' + msg.author.id + '> Unable to send you the help DM. You probably have DMs disabled.'));
				}
				return messages;
			} else if(commands.length > 15) {
				return msg.channel.send('<@' + msg.author.id + '> Multiple commands found. Please be more specific.');
			} else if(commands.length > 1) {
				return msg.channel.send(`<@` + msg.author.id + `> ` + disambiguation(commands, 'commands'));
			} else {
