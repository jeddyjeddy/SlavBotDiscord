...
class SwoleSpidermanCommand extends command.Command
 {
    ...
    async run(message, args)
    {
            ...
            var editText = args.toString()
                Jimp.read("swolespiderman.png").then(function (spidermanImage) {
                    Jimp.loadFont(Jimp.FONT_SANS_32_BLACK   ).then(function (font) {
                        spidermanImage.print(font, 16, 10, editText, 240).write(file, function(error){ 
        ...
        {
            if(args.length > 0)
                message.channel.send("<@" + message.author.id + "> A maximum of 55 characters are only allowed.").catch(error => {console.log("Send Error - " + error); });
            else
