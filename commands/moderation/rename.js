...
class RenameCommand extends command.Command
 {
    ...
    async run(message, args)
    {
        ...
        if(!message.guild.member(message.author).hasPermission("ADMINISTRATOR") && !message.guild.member(message.author).hasPermission("CHANGE_NICKNAME") && message.author.id != message.guild.owner.id){
            message.channel.send("<@" + message.author.id + "> This command is only available to the owner, or those with the Administrator or Change Nickname Permission.").catch(error => console.log("Send Error - " + error))
            return;
        }
