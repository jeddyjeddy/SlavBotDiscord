...
class KeanuCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "imageshit",
            memberName: "keanu",
            description: "Have Keanu show off your image. This command takes the last image uploaded, your avatar or the avatar of the user you have mentioned after the command.",
            examples: ["`!keanu`", "`!keanu avatar`", "`!keanu @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(args.toString().toLowerCase() != "avatar" && !otherUser)
        {
            ...
            .then(messages => {
                ...
                if(messageID == "")
                {
                    message.channel.send("<@" + message.author.id + "> No image found, use`" + commandPrefix + "help keanu` for help.").catch(error => {console.log("Send Error - " + error); });
                ...
                Jimp.read("Keanu.png").then(function (KeanuImage) {
                    ...
                    Jimp.read(url).then(function (userImage) {
                        ...
                        var mergedImage = blank.composite(KeanuImage, 0, 0);
