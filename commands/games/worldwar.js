                ...
                if(wars[i].key == message.guild.id)
                {
                    ...
                    if(wars[i].ended)
                    {
                        if(args.toLowerCase() == "start")
                        {
                            ...
                            message.channel.send("", {embed: {title: "***World War***", description: "New WW session has started.\n\n***Guide:***\nFight to conquer the world. Now that a game has started, it will not end until someone conquers every single country there is. Users can conquer countries using War Tokens. Users can also take countries from others with tokens (the value of countries increase based on the number of times it has been conquered). Users can gather resources using `" + commandPrefix + "ww collect` to earn War Tokens (the tokens can be used in WW games on other servers) that can be used to conquer different countries. These tokens can also be earned by voting for Slav Bot on discordbots.org (use `" + commandPrefix + "vote` to get the link) or by participating in token giveaways on the support server (use `" + commandPrefix + "support` to get the invite link). You can conquer any country by using `" + commandPrefix + "ww buy <country-name>`, you can get a list of all the countries using `" + commandPrefix + "ww list`. Don't be surprised if this game lasts for weeks, no one said you can conquer the world in a day. You can check your profile using `" + commandPrefix + "ww profile` to see how many tokens you have and which countries you have conquered. You can also check the local leaderboards for WW Games using `" + commandPrefix + "ww ranks`.\n\nGood luck everyone, may the best of you win.", color: 16711680, timestamp: timestamp, footer: {icon_url: message.client.user.avatarURL,text: "Started on"}}}).catch(error => console.log("Send Error - " + error));
                        }
                    ...
                    else
                    {
                        ...
                        {  
                            ...
                            {
                                message.channel.send("", {embed: {title: "***Cooldown***", description: "You cannot collect more resources until the 2 minute cooldown is over.", color: 65339, timestamp: IndexRef.getCooldown(message.author.id), footer: {icon_url: message.client.user.avatarURL,text: "Cooldown until"}}}).catch(error => console.log("Send Error - " + error));
                            }
                        ...
                        {
                            var countryName = args.toLowerCase().splice(3)
                            var notFound = true;
