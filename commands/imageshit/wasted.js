...
class Wasted extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            group: "imageshit",
            memberName: "wasted",
            description: "***WASTED.*** This command takes the last image uploaded, your avatar or the avatar of the user you have mentioned after the command.",
            examples: ["`!wasted`", "`!wasted avatar`", "`!wasted @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(args.toString().toLowerCase() != "avatar" && !otherUser)
        {
            ...
            .then(messages => {
                ...
                if(messageID == "")
                {
                    message.channel.send("<@" + message.author.id + "> No image found, use`" + commandPrefix + "help wasted` for help.").catch(error => {console.log("Send Error - " + error); });
                    
                    return;
                }
                message.channel.send("***taking image***").catch(error => {console.log("Send Error - " + error); });
                Jimp.read("wasted.png").then(function (WastedImage) {
