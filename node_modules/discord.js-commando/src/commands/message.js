...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.reply(`The \`${this.command.name}\` command must be used in a server channel.`).catch(error => console.log("Send Error - " + error));
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.reply(`The \`${this.command.name}\` command can only be used in NSFW channels.`).catch(error => console.log("Send Error - " + error));
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			if(typeof hasPermission === 'string') return this.reply(hasPermission).catch(error => console.log("Send Error - " + error));
			else return this.reply(`You do not have permission to use the \`${this.command.name}\` command.`).catch(error => console.log("Send Error - " + error));
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				if(result.prompts.length === 0) {
					...
					return this.reply(err.message).catch(error => console.log("Send Error - " + error));
				}
				return this.reply('Cancelled command.').catch(error => console.log("Send Error - " + error));
			}
			args = result.values;
		}
	...
	respond({ type = 'reply', content, options, lang, fromEdit = false }) {
		...
		switch(type) {
			...
			case 'reply':
				if(!shouldEdit) return this.message.reply(content, options).catch(error => console.log("Send Error - " + error));
				if(options && options.split && !options.split.prepend) options.split.prepend = `${this.message.author}, `;
				return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
			case 'direct':
				if(!shouldEdit) return this.message.author.send(content, options).catch(error => console.log("Send Error - " + error));
