...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.guildOnly && !this.message.guild) {
			...
			return this.channel.send(`The \`${this.command.name}\` command must be used in a server channel.`).catch(error => console.log("Send Error - " + error));
		}

		if(this.command.nsfw && !this.message.channel.nsfw) {
			this.client.emit('commandBlocked', this, 'nsfw');
			return this.channel.send(`The \`${this.command.name}\` command can only be used in NSFW channels.`).catch(error => console.log("Send Error - " + error));
		}
		...
		if(!hasPermission || typeof hasPermission === 'string') {
			...
			if(typeof hasPermission === 'string') return this.channel.send(hasPermission).catch(error => console.log("Send Error - " + error));
			else return this.channel.send(`You do not have permission to use the \`${this.command.name}\` command.`).catch(error => console.log("Send Error - " + error));
		}
		...
		if(!args && this.command.argsCollector) {
			...
			if(result.cancelled) {
				if(result.prompts.length === 0) {
					...
					return this.channel.send(err.message).catch(error => console.log("Send Error - " + error));
				}
				return this.channel.send('Cancelled command.').catch(error => console.log("Send Error - " + error));
			}
			args = result.values;
		}
	...
	respond({ type = 'reply', content, options, lang, fromEdit = false }) {
		...
		switch(type) {
			case 'plain':
				if(!shouldEdit) return this.message.channel.send(content, options).catch(error => console.log("Send Error - " + error));
			...
			case 'reply':
				...
				return this.editCurrentResponse(channelIDOrDM(this.message.channel), { type, content, options });
			case 'direct':
				if(!shouldEdit) return this.message.author.send(content, options).catch(error => {console.log("Send Error - " + error); this.message.channel.send(content, options).catch(error => console.log("Send Error - " + error))});
		...
		if(content instanceof Array) {
			...
			if(response instanceof Array) {
				for(let i = 0; i < content.length; i++) {
					...
					else promises.push(response[0].channel.send(`${prepend}${content[i]}`).catch(error => console.log("Send Error - " + error)));
				}
			} else {
				promises.push(response.edit(`${prepend}${content[0]}`, options));
				for(let i = 1; i < content.length; i++) {
					promises.push(response.channel.send(`${prepend}${content[i]}`).catch(error => console.log("Send Error - " + error)));
				}
			}
