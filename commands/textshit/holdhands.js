...
function grab_data(anon_id)
{
    ...
    // test search term
    var search_term = "anime-hold-hands";
...
class HoldHandsCommand extends command.Command
 {
    constructor(client)
    {
        super(client, {
            ...
            description: "Hold hands with yourself or another user.",
            examples: ["`!holdhands`", "`!holdhands @User`"]
        });
    }
    ...
    async run(message, args)
    {
        ...
        if(otherUser && userID != message.author.id)
        {
            if(gifs != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***and*** <@" + userID + "> ***are holding hands***", {
                    files: [url]
                }).catch(function (err) {
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***and*** <@" + userID + "> ***are holding hands***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
        ...
        else
        {
            if(gifsLonely != null)
            {
                ...
                message.channel.send("<@" + message.author.id + "> ***held their own hand***", {
                    files: [url]
                }).catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
            ...
            {
                message.channel.send("<@" + message.author.id + "> ***held their own hand***").catch(function (err) {
                    message.channel.send("Error - " + err.message).catch(error => console.log("Send Error - " + error));
                    console.log(err.message);
                });
                httpGetAsync(url,tenorCallback_anonid); 
