    ...
    async run(message, args)
    {
        ...
        if(args.toString().toLowerCase() != "avatar" && !otherUser)
        {
            ...
            .then(messages => {
                ...
                if(messageID == "")
                {
                    message.channel.send("<@" + message.author.id + "> No image found, use `" + commandPrefix + "help grayscale` for help.").catch(error => {console.log("Send Error - " + error); message.channel.stopTyping();});
