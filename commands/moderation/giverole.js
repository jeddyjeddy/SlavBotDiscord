            ...
            for(var i = 0; i < users.length; i++)
            {
                ...
                message.guild.fetchMember(user).then(function(member){
                    for(var roleIndex = 0; roleIndex < roles.length; roleIndex++)
                    {
                        ...
                        {
                            message.channel.send("<@" + message.author.id + "> You cannot give a role to the owner of the server via a command. This must be done manually.").catch(error => console.log("Send Error - " + error));
                        }
                        else
                        {
